...
const LOG_FILE = process.env.LOG_PATH ? path.resolve(process.env.LOG_PATH) : path.resolve(process.cwd(), 'log.txt');
...
async function initSFTP() {
    try {
        ...
        chokidar.watch(LOCAL_DIR, { persistent: true, ignoreInitial: true, depth: 0 })
            .on('add', filePath => {
        ...
        setInterval(descargarCSV, 10000); // cada 10 segundos
    } catch (err) {
        logMessage(`❌ Error SFTP: ${err.message}`);
        sftp.end();
    }
}
    ...
    try {
        ...
        for (const file of list) {
            if (file.name.toLowerCase().endsWith('.csv')) {
                ...
                if (!fs.existsSync(localPath)) { // solo descarga si no existe localmente
...
function procesarCSV(localPath) {
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            if (code === 0) {
                logMessage(`✅ Archivo procesado correctamente: ${path.basename(localPath)}`);
                resolve();
            } else {
                logMessage(`❌ Error procesando archivo: ${path.basename(localPath)} (code ${code})`);
